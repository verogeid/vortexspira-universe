/* --- code/features/navigation/nav-keyboard-base.js --- */
...
export function initKeyboardControls() {
    ...
    document.addEventListener('mousedown', (e) => {
        ...
        debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.EXTREME, 
            `[TRACE: ${this.STATE.currentTraceId}] 🖱️ Mousedown capturado en:`, 
            ...
            if (!isInteractive) {
                ...
                if (container.id !== 'vista-central') {
                    ...
                    if (focusables[lastIndex]) {
                        debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.EXTREME, 
                            `🛡️ Escudo: Clic en zona muerta. ` +
                            `Forzando foco en el índice [${lastIndex}]`);
                    ...
                    if (document.activeElement && document.activeElement !== document.body) {
                        ...
                        debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.EXTREME, 
                            `🖱️ Clic en Swiper: Forzando blur del elemento anterior ` +
    ...
    document.addEventListener('focusin', (e) => {
        ...
        debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.EXTREME, 
            `[TRACE: ${traceInfo}] 🎯 Focusin disparado para:`, 
            e.target.tagName, e.target.id);
        ...
        if (container) {
            ...
            if (isInteractive && container.id !== 'vista-central') {
                ...
                if (index !== -1) {
                    ...
                    debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.EXTREME, 
                        `📝 Notario: Guardado índice [${index}] ` +
                        `en la zona '${app.STATE._lastActiveZoneId}'`);
                }
            }
        }
    });
...
function _handleGlobalWheel(e) {
    ...
    if (e.deltaY !== 0) {
        ...
        if (isNavActive) {
            debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.EXTREME, 
                `🐭 [WHEEL TRACE 2] Pasó los bloqueos. ` +
                `Enviando [${key}] a _handleSwipeNavigation.`);
...
export function _handleFocusTrap(e, viewType) {
    ...
    debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.BASIC, 
        ...
        `- Zona rescatada de memoria: ` +
                       `${wasRecovered ? 'SÍ (' + app.STATE._lastActiveZoneId + ')' : 'NO'} ` +
    ...
    if (isLostFocus && !e.shiftKey) {
        ...
        debug.log('nav_keyboard_base', debug.DEBUG_LEVELS.BASIC, 
            `🧠 Rescate de memoria activado para grupo index [${groupIdx}]. ` +
            `Intentando enfocar el último elemento conocido...`);
