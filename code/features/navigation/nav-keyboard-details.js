// --- code/features/navigation/nav-keyboard-details.js ---

import * as nav_base_details from './nav-base-details.js'; 
import * as debug from '../../debug/debug.js'; 
import * as data from '../../services/data.js'; // Necesario para data.SWIPER.SLIDE_SPEED
...
export function _handleDetailNavigation(key) {
    ...
    debug.log('nav_keyboard_details', debug.DEBUG_LEVELS.BASIC, 
        `\n[TRACE ${app.STATE.currentTraceId}] ⌨️ Pulsada tecla ${key}. ` +
        `Flag keyboardNavInProgress = TRUE`);
    ...
    debug.log('nav_keyboard_details', debug.DEBUG_LEVELS.DEEP, 
        `[TRACE ${app.STATE.currentTraceId}] Current Focus Index (List): ${currentIndex}, ` +
            ...
            if (targetElement) {
                ...
                if (targetElement.classList.contains('detail-text-fragment') || 
                    ...
                    debug.log('nav_keyboard_details', debug.DEBUG_LEVELS.DEEP, 
                        ...
                        `Avanzando al siguiente fragmento.`);
    ...
    if (newIndex !== currentIndex) {
        ...
        if (newFocusElement) {
            ...
            debug.log('nav_keyboard_details', debug.DEBUG_LEVELS.BASIC, 
                `[TRACE ${app.STATE.currentTraceId}] 4. Llamando sincrónicamente a ` +
                `_updateDetailFocusState`);
        ...
        debug.log('nav_keyboard_details', debug.DEBUG_LEVELS.BASIC, 
            `[TRACE ${app.STATE.currentTraceId}] Índice sin cambios. ` + 
            `Flag keyboardNavInProgress = FALSE`);
    }
}
...
function _checkAndFixVerticalObstruction(app, target, swiper, traceId = 'NA') {
    ...
    debug.log('nav_keyboard_details', debug.DEBUG_LEVELS.BASIC, 
        `[TRACE ${traceId}-CHK] 🔧 EJECUTANDO setTranslate secundario: ${reason}. ` +
        `Físico: ${currentTrans.toFixed(1)} -> Destino: ${newTrans.toFixed(1)}`);
