...
export function handleSlideChangeStart(swiper) {
    ...
    if (swiper.activeIndex !== swiper.previousIndex) {
        ...
        debug.log('nav_mouse_swipe', debug.DEBUG_LEVELS.DEEP, 
            `⚡️ START SlideChange. Dir: ${_swipeDirection} | ` +
            `Prev: ${swiper.previousIndex} -> Act: ${swiper.activeIndex}`);
    }
};
...
export function handleSlideChangeEnd(swiper) {
    ...
    if (this.STATE.forceFocusRow !== undefined && this.STATE.forceFocusRow !== null) {
        ...
        debug.log('nav_mouse_swipe', debug.DEBUG_LEVELS.DEEP, 
            `Calculando targetRow: idx(${currentFocusIndex}) ` +
            `% cols(${itemsPorColumna}) = ${targetRow}`);
    }
    ...
    if (newFocusCard) {
        ...
        debug.log('nav_mouse_swipe', debug.DEBUG_LEVELS.DEEP, 
            `Candidato Foco: ID=${newFocusCard.dataset.id} | ` +
            `LogicalPos=${newLogicalIndex} | ` +
            `CurrentIdx=${this.STATE.currentFocusIndex}`);
