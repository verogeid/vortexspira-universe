...
export function setupNavigationListener(app) {
    window.addEventListener('popstate', (event) => {
        ...
        if (targetLang && targetLang !== currentLang) {
            debug.log('app', debug.DEBUG_LEVELS.BASIC, `Cambio de idioma detectado vía Back/Next: ${targetLang}`);
            window.location.reload(); 
            return;
        }

        debug.log('app', debug.DEBUG_LEVELS.BASIC, `Navegación nativa detectada. Destino ID: ${targetId} | Lang: ${targetLang}`);
