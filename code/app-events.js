...
export function setupGlobalListeners(app) {
    ...
    const bootKeyboardModule = async (e) => {
        ...
        try {
            ...
            debug.log('app', debug.DEBUG_LEVELS.BASIC, '⌨️/⚙️ Teclado o Rueda detectado. Módulo inyectado en caliente.');
        } catch (err) {
    ...
    const bootPointerModule = async () => {
        ...
        try {
            ...
            debug.log('app', debug.DEBUG_LEVELS.BASIC, '🖱️/👆 Puntero/Touch detectado. Módulo de swipe inyectado.');
        } catch (err) {
    ...
    window.addEventListener('vortex-layout-refresh', () => {
        ...
        if (forceRender) {
            debug.log('app', debug.DEBUG_LEVELS.BASIC, `Layout Change (${prevMode} -> ${newMode}). Forzando render...`);
            app._cacheDOM();
        } else {
            debug.log('app', debug.DEBUG_LEVELS.DEEP, `Layout Refresh (Mismo modo). Delegando a buckets.`);
        }
