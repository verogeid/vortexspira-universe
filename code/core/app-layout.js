...
export function updateLayoutMode(app) {
    ...
    if (currentMode !== candidateMode) {
        ...
        injectCSS(`styles/layouts/style-${cssFile}.css`, 'vortex-css-layout');
        debug.log('app', debug.DEBUG_LEVELS.BASIC, 
            `Layout Final: ${candidateMode} ` + 
            `(W:${effectiveWidth.toFixed(0)} / H:${effectiveHeight.toFixed(0)})`);
    }
    ...
    if (enableSafeMode !== currentSafe) {
        ...
        if (enableSafeMode) {
            injectCSS('styles/layouts/style-safe-mode.css', 'vortex-css-safe');
        } else {
        ...
        debug.log('app', debug.DEBUG_LEVELS.BASIC, 
            `🛡️ Safe Mode: ${enableSafeMode ? 'ON' : 'OFF'} (Solo Mobile) ` + 
            `| Eff.Height: ${effectiveHeight.toFixed(0)}px`);
