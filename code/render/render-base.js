...
export function renderNavegacion() {
    ...
    debug.log('render_base', debug.DEBUG_LEVELS.EXTREME, 
        `[RENDER-FLOW] 01. START renderNavegacion. ` +
        `FullData:${!!this.STATE.fullData} Hydrating:${this.STATE.isHydrating}`);
    ...
    if (itemsDelNivel.length === 0) {
        itemsDelNivel.push({
            ...
            nombre: this.getString('nav.emptySection')
        });
    }
    ...
    if (this.STATE.resizeSnapshot) {
        ...
        if (foundEl) {
            debug.log('render_base', debug.DEBUG_LEVELS.EXTREME, 
                `[RENDER-FLOW] 03A. Exact Match Found: ${snap.value}. ` +
                `Setting explicitExternalFocus.`);
                ...
                if (explicitExternalFocus) {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            debug.log('render_base', debug.DEBUG_LEVELS.EXTREME, 
                                `[RESTORE-DEBUG] 08. Re-Check: IsFocused=${isFocused}, ` +
                                `HasClass=${hasClass}`);
...
export function _generarTarjetaHTMLImpl(nodo, 
    ...
    const actionPrefix = isCourse 
        ? (this.getString('nav.aria.actionCourse')) 
        : (this.getString('nav.aria.actionCategory'));
...
export function _updateNavViews(isSubLevel, isMobile, isTabletPortrait, 
    ...
    const contextPrefix = this.getString('nav.contextPrefix');
